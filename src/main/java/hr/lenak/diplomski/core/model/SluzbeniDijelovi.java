...
/**
 * The persistent class for the SLUZBENI_DIJELOVI database table.
 * 
 */
@Entity
@Table(name="SLUZBENI_DIJELOVI")
public class SluzbeniDijelovi implements Serializable {
	...
	private List<TekstoviSluzbeni> tekstoviSluzbenis;
	
	@Id
	@SequenceGenerator(name="SLUZBENI_DIJELOVI_SDOID_GENERATOR", sequenceName="SEQUENCEGENERATOR")
	@GeneratedValue(strategy=GenerationType.SEQUENCE, generator="SLUZBENI_DIJELOVI_SDOID_GENERATOR")
	...
	//bi-directional many-to-one association to TekstoviSluzbeni
	@OneToMany(mappedBy="sluzbeniDijelovi")
	public List<TekstoviSluzbeni> getTekstoviSluzbenis() {
		return this.tekstoviSluzbenis;
	}

	public void setTekstoviSluzbenis(List<TekstoviSluzbeni> tekstoviSluzbenis) {
		this.tekstoviSluzbenis = tekstoviSluzbenis;
	}

	public TekstoviSluzbeni addTekstoviSluzbeni(TekstoviSluzbeni tekstoviSluzbeni) {
		getTekstoviSluzbenis().add(tekstoviSluzbeni);
		tekstoviSluzbeni.setSluzbeniDijelovi(this);

		return tekstoviSluzbeni;
	}

	public TekstoviSluzbeni removeTekstoviSluzbeni(TekstoviSluzbeni tekstoviSluzbeni) {
		getTekstoviSluzbenis().remove(tekstoviSluzbeni);
		tekstoviSluzbeni.setSluzbeniDijelovi(null);
	...
	public boolean equals(Object obj) {
		...
		if (getClass() != obj.getClass())
			return false;
		SluzbeniDijelovi other = (SluzbeniDijelovi) obj;
		if (sdoId == null) {
			if (other.sdoId != null) {
				return false;
			}
